...
def test_tcp_check_success(health_check_service, monkeypatch):
    ip = "127.0.0.1"
    port = 8080
    timeout_seconds = 5
...
def test_tcp_check_failure(health_check_service):
    ip = "127.0.0.1"
    port = 8888
    timeout_seconds = 5
...
def test_http_check_success(health_check_service, monkeypatch):
    ip = "127.0.0.1"
    scheme = "http"
    port = "8080"
    path = "/health"
...
def test_http_check_failure(health_check_service, monkeypatch):
    ip = "127.0.0.1"
    scheme = "http"
    port = "8888"
    path = "/health"
...
def test_check_tcp_protocol(health_check_service, monkeypatch):
    ...
    mock_tcp_check.return_value = HealthCheckResultModel(healthy=True)
...
def test_check_http_protocol(health_check_service, monkeypatch):
    destination = "127.0.0.1"
    scheme = "http"
    port = 8080
    path = "/health"
    ...
    mock_http_check.return_value = HealthCheckResultModel(healthy=True)
...
def test_check_unsupported_protocol(health_check_service):
    destination = "127.0.0.1"
    sg_dns_config = MagicMock()
